...
@Component({
  ...
  templateUrl: './go-reset-receipt.component.html',
  styleUrl: './go-reset-receipt.component.css'
})
export class GoResetReceiptComponent implements OnInit,AfterViewInit, OnDestroy {
  ...
  private subListLPO!:Subscription;
    ...
    ngAfterViewInit() {     
        ...
        .pipe(
          ...
          switchMap(() => {
            ...
            return this.dataService!.getPODetailsFromFleet4ResetReceiptXport(
        ...
        .subscribe(data => {
          ...
          if (this.data.length==0){
            ...
            this.dataService.appMessageDialogContent.set(`No rr found; The LPO# ${this.LpoRef} might have exported to GRP earlier`);
  ...
  onSubmit() {
    ...
    if (this.lpoResetPostingForm.valid) {
      ...
      this.subPostLPO = this.dataService.resetReceipts2GRP(this.reset_lpo_request).subscribe((resp)=>{


        this.spinner.hide();
       
        this.dataService.appMessageDialogContent.set(`The receipts of LPO# ${this.LpoRef} has been reset Successfully `);
